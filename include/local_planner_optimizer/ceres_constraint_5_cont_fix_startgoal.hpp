#ifndef CERES_CONSTRAINTS_5_CONT_FIX_STARTGOAL
#define CERES_CONSTRAINTS_5_CONT_FIX_STARTGOAL
...
class Ceres5_FixStartGoalContFunctor {

public:
    Ceres5_FixStartGoalContFunctor(double weight, Planners::utils::Vec3i local_start, Planners::utils::Vec3i local_goal): weight_(weight), local_start_(local_start), local_goal_(local_goal) {}
    ...
    bool operator()(const T* const stateCoeff, T* residual) const {
        ...
        //Planners::utils::safe_log("Cost Function - Fix Start/Goal", dt.count());
        //std::cout << "[T] - Cost Function - Fix Start/Goal: " << dt.count() << std::endl;
