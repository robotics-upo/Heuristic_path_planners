...
#pragma GCC diagnostic pop
	...
	void computeGrid(void)
	{
		...
		// m_gridSizeX = (int)(211.8*m_oneDivRes); //1059
		// m_gridSizeY = (int)(169.2*m_oneDivRes); //846
		// m_gridSizeZ = (int)(9.2*m_oneDivRes);  //46
		...
		std::cout << "Size X: " << m_gridSizeX << std::endl; //1059
		std::cout << "Size Y: " << m_gridSizeY << std::endl; //846
		std::cout << "Size Z: " << m_gridSizeZ << std::endl; //46

		std::cout << "m_max X: " << m_maxX << std::endl; //211.8
		std::cout << "m_may Y: " << m_maxY << std::endl; //169.2
		std::cout << "m_maz Z: " << m_maxZ << std::endl; //9.2
		...
		for(int iz=0; iz<(m_gridSizeZ-2); iz++) //m_gridSizeZ-2? Para que coincida con el script
		{
			for(int iy=0; iy<(m_gridSizeY-3); iy++) //m_gridSizeY-3? Para que coincida con el script
			{
				for(int ix=0; ix<(m_gridSizeX); ix++)
				{
