...
private:
    ...
    void calculatePath3D()
    {
        ...
        origen_local_x_cont = drone_x_ - drone_local_x * resolution_;
        origen_local_y_cont = drone_y_ - drone_local_y * resolution_;
        ...
        for (const auto& vec : global_path_) 
        {
            ...
            newpoint.x = vec.x * global_path_original_resolution / resolution_ - origen_local_x; //Accounts for difference in resolution between global and local systems
            newpoint.y = vec.y * global_path_original_resolution / resolution_ - origen_local_y;
            newpoint.z = vec.z * global_path_original_resolution / resolution_ - origen_local_z;
        ...
        if(USING_CERES){
            ...
            {
                // 1 - Calculate initial parameters for the 3th grade functions
                ...
                else if(CONTINUOUS_FUNCTION_INITIAL_PARAMETERS == 1) // GLOBAL PATH APPROXIMATION
                {
                    ...
                    init_coeff_x << 0, 0, x_coeff_1, drone_local_x;
                    init_coeff_y << 0, 0, y_coeff_1, drone_local_y;
                    init_coeff_z << 0, 0, z_coeff_1, drone_local_z;
                ...
                for(int i=0; i < N_DIVISIONS_INI + 1; i++)
                {
                    ...
                    global_wp_point.x = coeff_x(0) * std::pow(t_act, 3) + coeff_x(1) * std::pow(t_act, 2) + coeff_x(2) * t_act + coeff_x(3) + origen_local_x;
                    global_wp_point.y = coeff_y(0) * std::pow(t_act, 3) + coeff_y(1) * std::pow(t_act, 2) + coeff_y(2) * t_act + coeff_y(3) + origen_local_y;
                    global_wp_point.z = coeff_z(0) * std::pow(t_act, 3) + coeff_z(1) * std::pow(t_act, 2) + coeff_z(2) * t_act + coeff_z(3) + origen_local_z;
                ...
                for(int i=0; i < N_DIVISIONS + 1; i++)
                {
                    ...
                    Planners::utils::Vec3i global_wp_point;
                    global_wp_point.x = opt_local_path_function.x_params[0] * std::pow(t_act, 3) + opt_local_path_function.x_params[1] * std::pow(t_act, 2) + opt_local_path_function.x_params[2] * t_act + opt_local_path_function.x_params[3] + origen_local_x;
                    global_wp_point.y = opt_local_path_function.y_params[0] * std::pow(t_act, 3) + opt_local_path_function.y_params[1] * std::pow(t_act, 2) + opt_local_path_function.y_params[2] * t_act + opt_local_path_function.y_params[3] + origen_local_y;
                    global_wp_point.z = opt_local_path_function.z_params[0] * std::pow(t_act, 3) + opt_local_path_function.z_params[1] * std::pow(t_act, 2) + opt_local_path_function.z_params[2] * t_act + opt_local_path_function.z_params[3] + origen_local_z;
                ...
                // Pointcloud de obstáculos (coordenadas locales)
                std::vector<Eigen::Vector3d> obst_point_cloud = {
                    Eigen::Vector3d(17.0, 10.0, 8.0),
                    Eigen::Vector3d(17.0, 10.0, 6.0),
                    Eigen::Vector3d(17.0, 10.0, 10.0),
                    Eigen::Vector3d(17.0, 10.0, 4.0),
                    Eigen::Vector3d(17.0, 10.0, 12.0),
                    //Eigen::Vector3d(0.0, -1.5, 0.0),
                    //Eigen::Vector3d(-2.0, 0.5, 1.0),
                    //Eigen::Vector3d(1.5, 1.5, 1.5),
                    Eigen::Vector3d(13.0, 5.0, 8.0),
                    Eigen::Vector3d(13.0, 5.0, 6.0),
                    Eigen::Vector3d(13.0, 5.0, 10.0),
                    Eigen::Vector3d(13.0, 5.0, 4.0),
                    Eigen::Vector3d(13.0, 5.0, 12.0)
                };

                for(const auto& obs : obst_point_cloud) {
                    geometry_msgs::Point ret;
                    ret.x = (obs[0] + origen_local_x) * resolution_;
                    ret.y = (obs[1] + origen_local_y) * resolution_;
                    ret.z = (obs[2] + origen_local_z) * resolution_;
                    obstacle_pc_points_markers_.points.push_back(ret);
                }
                publishMarker(obstacle_pc_points_markers_, obstacle_pc_point_markers_pub_);
                obstacle_pc_points_markers_.points.clear();



                Planners::utils::OptimizedContinuousFunction opt_local_path_function;
                auto ceres_start = std::chrono::high_resolution_clock::now();
                opt_local_path_function = Ceresopt::ceresOptimizerTestContinuousPath(coeff_x, coeff_y, coeff_z, local_goal, resolution_, obst_point_cloud);
                auto ceres_stop = std::chrono::high_resolution_clock::now();
                std::chrono::duration<double, std::milli> ceres_duration = ceres_stop - ceres_start;
                printf("TIEMPO TOTAL DEL OPTIMIZADOR: %.2f ms\n", ceres_duration.count());
    ...
    void configMarkers(const std::string &_ns, const std::string &_frame, const double &_scale){
        ...
        obstacle_pc_points_markers_.ns = _ns;
        obstacle_pc_points_markers_.header.frame_id = _frame;
        obstacle_pc_points_markers_.header.stamp = ros::Time::now();
        obstacle_pc_points_markers_.id = rand();
        obstacle_pc_points_markers_.lifetime = ros::Duration(500);
        obstacle_pc_points_markers_.type = visualization_msgs::Marker::POINTS;
        obstacle_pc_points_markers_.action = visualization_msgs::Marker::ADD;
        obstacle_pc_points_markers_.pose.orientation.w = 1;
        obstacle_pc_points_markers_.color.r = 1.0;
        obstacle_pc_points_markers_.color.g = 0.0;
        obstacle_pc_points_markers_.color.b = 1.0;
        obstacle_pc_points_markers_.color.a = 1.0;
        obstacle_pc_points_markers_.scale.x = _scale;
        obstacle_pc_points_markers_.scale.y = _scale;
        obstacle_pc_points_markers_.scale.z = _scale;
    ...
    void publishMarker(visualization_msgs::Marker &_marker, const ros::Publisher &_pub){
        ...
        if( !overlay_markers_ ){
            ...
            obstacle_pc_points_markers_.id           = rand();
            obstacle_pc_points_markers_.header.stamp = ros::Time::now();
            setRandomColor(obstacle_pc_points_markers_.color);
    ...
    visualization_msgs::Marker local_path_line_markers_, local_path_points_markers_, local_path_velocity_markers_, ini_local_path_line_markers_, ini_local_path_points_markers_, obstacle_pc_points_markers_;
